---
title: Excel conventions in the wild
description: A catalog of weird patterns customers ship in spreadsheets, and how Crease handles each.
---

import { ExcelGrid, Caption, Convention, CreaseHandles } from './doc-components'
...
The examples below use a custom `<ExcelGrid>` React component that renders
HTML tables styled to look like Excel. The component is intentionally simple
— see `doc-components.tsx` for a reference implementation. The doc also reads
correctly on GitHub (which strips the JSX) because each example falls back
to a plain HTML table.
...
<ExcelGrid headers={["A"]} rows={[
  ["Acme Corporation — Order Export"],
  ["Generated: 2025-04-15"],
  [""],
  ["order_id", "order_date", "customer_email"],
  ["ORD-1001", "2025-01-15", "a@acme.com"],
]} headerRowIndex={3} />
...
<CreaseHandles>
  `locate.header_row: 3` (zero-indexed) and `locate.data_starts_row: 4`. The
  LLM is expected to recognize that rows 0-2 are non-tabular and pick the
  correct header row from the samples.
</CreaseHandles>
...
<CreaseHandles>
  `locate.data_ends_at: {type: value_match, column: 0, value: "TOTAL"}` —
...
<table>
  ...
  <tbody>
    <tr><td>1</td><td>SKU</td><td colSpan="3" style={{textAlign:"center"}}>Q1 2025</td></tr>
...
<CreaseHandles>
  **Deferred to v2.** This requires either a crosstab-aware extractor or a
  pre-flatten step. v1 will see this as `column_count_mismatch` and emit a
  structural error suggesting re-onboarding.
</CreaseHandles>
...
<CreaseHandles>
  `orientation: anchored` with per-field `anchor.label_match`. Each field
  finds its label independently, walks one cell right, reads the value.
</CreaseHandles>
...
<CreaseHandles>
  **Deferred to v1.5.** v1 supports one entity per tab. To handle this
  reliably we'll add `locate.cell_range` so each entity can be bound to a
  rectangular sub-region of the tab.
</CreaseHandles>
...
<CreaseHandles>
  **Deferred to v1.5.** Would require a row-skip predicate
  (e.g. `skip_row_if: cell_at(column: 0) matches "/subtotal/i"`). For v1
  we expect customers to remove subtotals or for us to flag them as
  `wrong_type` rows since the subtotal cells often don't match field types.
</CreaseHandles>
...
<CreaseHandles>
  Pandera coerces the dominant format; outliers emit `wrong_type` per row.
  For columns with strongly mixed encodings, prefer `type: string` and
  parse downstream; or add `pattern:` to enforce one shape and route bad
  rows to human review.
</CreaseHandles>
...
<table>
  ...
  <tbody>
    <tr><td>1</td><td><span style={{color:"#666"}}>'</span>00123</td><td>10</td></tr>
    <tr><td>2</td><td><span style={{color:"#666"}}>'</span>00124</td><td>20</td></tr>
  </tbody>
</table>

<CreaseHandles>
  pandas reads these as strings. If the field is `type: string`, no problem.
  If it's `type: integer`, Pandera coercion may succeed (it strips the
  leading apostrophe) or emit `wrong_type` if there are non-numeric chars.
</CreaseHandles>
...
<CreaseHandles>
  v1: each non-empty sentinel triggers `wrong_type` (won't match email regex).
  v1.5: add `null_tokens:` to field spec — list of strings to treat as null
  during extraction. For now, encourage operators to leave cells truly blank.
</CreaseHandles>
...
<table>
  ...
  <tbody>
    <tr><td>1</td><td style={{textAlign:"right"}}>1,234.56</td></tr>
    <tr><td>2</td><td style={{textAlign:"right"}}>(842.10)</td></tr>
    <tr><td>3</td><td style={{textAlign:"right"}}>500.00</td></tr>
  </tbody>
</table>

<CreaseHandles>
  Excel's number formatting hides the sign, but the underlying value is
  numeric. pandas + openpyxl read the actual stored value, not the display.
  Only an issue when the cell is *text-stored* as `"(842.10)"` — then
  `wrong_type` fires.
</CreaseHandles>
...
<table>
  ...
  <tbody>
    <tr><td>1</td><td style={{textAlign:"right"}}>$1,234.56</td></tr>
    <tr><td>2</td><td style={{textAlign:"right"}}>€90.00</td></tr>
    <tr><td>3</td><td style={{textAlign:"right"}}>£75.50</td></tr>
  </tbody>
</table>

<CreaseHandles>
  Same as above — depends on whether Excel stored a number or a string.
  Customers who mix currencies in one column are flagging a real semantic
  problem; consider a `currency` field alongside `amount`.
</CreaseHandles>
...
<CreaseHandles>
  v1: `type: boolean` + `enum:` listing the accepted forms. v1.5: add a
  built-in `boolean_aliases:` per field.
</CreaseHandles>
...
<Caption>
Operator A: title case. Operator B: snake_case. Operator C: title case with a
trailing space and a non-breaking space between words.
</Caption>

<CreaseHandles>
  Extractor normalizes headers (trim, NBSP→space, lower-case) before matching
  `source_column`. The template's `source_column: "Order ID"` matches all three.
</CreaseHandles>
...
<CreaseHandles>
  Stays as a single string with `\n` separators. If the consumer wants
  it split, that's a downstream concern.
</CreaseHandles>
...
<CreaseHandles>
  Crease reads hidden cells as if they were visible. If a customer
  intentionally hides "draft" rows, those will appear in extraction.
  v1.5: option `skip_hidden: true` on the locate spec.
</CreaseHandles>
...
<CreaseHandles>
  List them in `ignore_tabs:`. The LLM is expected to spot common names
  (`Notes`, `Instructions`, `Cover Page`, `Sheet1` when empty) during
  inference and add them automatically.
</CreaseHandles>
...
<CreaseHandles>
  **Deferred to v2.** This is a meta-template that Crease could in principle
  parse first to learn which tabs to look at. For v1, the operator declares
  the pattern in `tab_pattern` directly.
</CreaseHandles>
...
<CreaseHandles>
  **Deferred to v1.5.** Adds `skip_row_if: matches_header` to drop rows
  whose values equal the header text.
</CreaseHandles>
...
<CreaseHandles>
  `openpyxl.load_workbook(path, data_only=True)` reads the cached value
  Excel computed, not the formula text. Crease uses `data_only=True`
  throughout. If a customer sends a file that was never opened in Excel
  (cache is stale), the cell may come back as `None` or the formula
  string — emit `extraction_failed` with a message suggesting they
  open + save the file in Excel.
</CreaseHandles>
...
<CreaseHandles>
  Tab patterns can be loose: `tab_pattern: ^Orders` matches any of those.
  Better: a v1.5 fuzzy-match mode that the LLM picks based on examples.
</CreaseHandles>
...
## Reference: the `<ExcelGrid>` component

```tsx
// doc-components.tsx — sketch
export function ExcelGrid({headers, rows, headerRowIndex}) {
  return (
    <table className="xlsx-grid">
      <thead><tr><th></th>{headers.map(h => <th>{h}</th>)}</tr></thead>
      <tbody>
        {rows.map((r, i) => (
          <tr className={i === headerRowIndex ? "is-header-row" : ""}>
...
export function CreaseHandles({children}) {
  return <aside className="crease-handles">
    <strong>How Crease handles it →</strong> {children}
  </aside>
}
```

Style as desired. The plain HTML `<table>` fallbacks in this file are
intentional so the doc reads on GitHub without a build step.
