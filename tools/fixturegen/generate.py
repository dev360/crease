...
from layouts import (
    ...
    render_crosstab,
    render_flat,
    render_property_sheet,
    render_transposed,
)
from series import CROSSTABS, SERIES
...
def _render_series(series_name: str, layout_name: str, n_records: int, rng: random.Random):
...
def _save(sheet, label: dict, scenario_id: str, source: str, layout_name: str, out_dir: Path):
...
def _maybe_corrupt(sheet, layout_name: str, rng: random.Random):
    if rng.random() < CLEAN_RATIO:
        return corruptors.CorruptionResult(
            sheet,
            {
                "verdict": "valid",
                "bad_rows": [],
...
def generate(out_dir: Path, per_combo: int, rows_per_file: int, seed: int):
    ...
    summary = {"total": 0, "by_source": {}, "by_layout": {}, "by_reason": {}, "by_verdict": {}}
    ...
    for source_type, source_name, layout_name in combos:
        for i in range(per_combo):
            ...
            if source_type == "series":
                sheet = _render_series(source_name, layout_name, rows_per_file, scenario_rng)
            else:
            ...
            label = _save(result.sheet, result.label, scenario_id, source_name, layout_name, out_dir)

            counter += 1
            summary["total"] += 1
            summary["by_source"][source_name] = summary["by_source"].get(source_name, 0) + 1
            summary["by_layout"][layout_name] = summary["by_layout"].get(layout_name, 0) + 1
            reason_key = label["reason"] or "_clean"
            summary["by_reason"][reason_key] = summary["by_reason"].get(reason_key, 0) + 1
            summary["by_verdict"][label["verdict"]] = summary["by_verdict"].get(label["verdict"], 0) + 1
...
def main():
    ...
    p.add_argument("--per-combo", type=int, default=20, help="files per (source × layout) combination")
    p.add_argument(
        "--rows-per-file",
        type=int,
        default=50,
